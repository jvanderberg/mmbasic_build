MODE 2  '(HDMI patch: must precede any MM.HRES/MM.VRES read)
' ==========================================
...
' ---- Tunables ----
CONST PAD_ACCEL = 0.88                 ' was 2.0  (60fps tune)
CONST PAD_DECAY = 0.945                ' was 0.88 (60fps tune)
CONST PAD_MAX   = 10.5                 ' was 24   (60fps tune)
CONST BRADIUS   = 6
CONST BALL_SPEED_INIT = 1.32           ' was 3.0  (60fps tune)
CONST BALL_SPEED_ACCEL_PER_SEC = 0.013 ' was 0.03 (60fps tune)
CONST PADDLE_KICK = 0.2                ' (reverted: pvx! already scaled; was double-scaled)
CONST TICK_MS   = 38
...
CONST BLOCK_COLS% = 8
CONST BLOCK_GAP% = 4
CONST BLOCK_W% = (W% - (BLOCK_COLS% + 1) * BLOCK_GAP%) \ BLOCK_COLS%   ' was 35; scales to fit width
CONST BLOCK_H% = BLOCK_W% \ 3                                      ' was 12; keeps ~2.9:1 aspect
CONST BLOCK_TOP% = 40
...
' ---- Setup Framebuffers ----
' (HDMI patch: F unused; saves 96KB internal RAM for L) FRAMEBUFFER CREATE
...
' (HDMI: hardware composites L over F automatically) FRAMEBUFFER MERGE RGB(BLACK), R, TICK_MS
...
DO
  ...
  IF k$ <> "" THEN
    SELECT CASE ASC(k$)
      ...
      CASE 80, 112: ' P or p key - screenshot
        ...
        ' (HDMI: hardware composites L over F automatically) FRAMEBUFFER MERGE RGB(BLACK), A  ' Abort continuous merge
        PAUSE 50
        FRAMEBUFFER WRITE N
        CLS COL_BG%
        DrawBlocks
        ...
        PAUSE 50
        FRAMEBUFFER WRITE L
        ' (HDMI: hardware composites L over F automatically) FRAMEBUFFER MERGE RGB(BLACK), R, TICK_MS  ' Resume continuous merge
  ...
  IF ballLaunched% = 1 AND INT(by!) + 2*br% >= INT(py!) AND INT(by!) <= INT(py!) + ph% _
    ...
    angle! = (hitPos! - 0.5) * 2.2  ' Range: -1.1 to +1.1 (60fps scaled, was 5.0)
  ...
  IF ballLaunched% = 1 AND INT(by!) > H% - 2*br% THEN
    ...
    ELSE
      ' Game over
      ' (HDMI: hardware composites L over F automatically) FRAMEBUFFER MERGE RGB(BLACK), A
      FRAMEBUFFER CLOSE
      FRAMEBUFFER WRITE N
      CLS COL_BG%
  ...
  FRAMEBUFFER WAIT          '(moved: redraw during vblank)
  ' ---- Redraw layer with new sprite positions ----
  ' (HDMI: no-op) FRAMEBUFFER SYNC
  FRAMEBUFFER WRITE L
  CLS RGB(BLACK)    ' Clear layer with transparent color
  DrawPaddleAt INT(px!), INT(py!)
  DrawBallAt INT(bx!), INT(by!)
  ...
  IF TIMER - t0% >= 1000 THEN
    ...
    DrawHUD
  END IF
  '---- Frame pacing: PAUSE remainder of TICK_MS after WAIT (~16ms vsync) ----
LOOP
